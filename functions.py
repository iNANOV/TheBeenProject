...
def plot_signal_chart(df, selection, signal):
   ...
   # df["Signal"] = df.filter(regex="^S\d+").apply(pd.to_numeric, errors="coerce").sum(axis=1)
    ...
    # Ensure 'date' is in datetime format before proceeding
    df['date'] = pd.to_datetime(df['date'], errors='coerce')  # Coerce any invalid date formats to NaT

    # Blue scale: Light blue for low values, dark blue for high values
    bar_cmap = cm.get_cmap("coolwarm")  # Use a predefined colormap for better color range

    # Bar Chart: Adjusting color and aligning dates
    fig1, ax1 = plt.subplots(figsize=(12, 6))

    # Normalize and plot bars
    norm = Normalize(vmin=df["Signal"].min(), vmax=df["Signal"].max())
    bars = ax1.bar(df["date"], df["R_t"], color=bar_cmap(norm(df["Signal"])), width=0.8)

    # Set x-axis ticks to show a limited number of dates to avoid overcrowding
    # Set tick positions every 5th date, or choose a more appropriate interval
    ax1.set_xticks(df['date'][::max(1, len(df)//10)])  # Set ticks at fixed intervals based on the length of the dataframe
    ax1.set_xticklabels(df['date'].dt.strftime('%Y-%m-%d')[::max(1, len(df)//10)], rotation=90, ha='center', fontsize=9)
...
def plot_signal_chart_new(df, selection, signal):
    # Ensure 'date' is in datetime format
    df['date'] = pd.to_datetime(df['date'], errors='coerce')

    # Blue scale: Light blue for low values, dark blue for high values (keeping your original choice)
    bar_cmap = cm.get_cmap("coolwarm")  # Use a predefined colormap for better color range
    ...
    # Set x-axis ticks to show all the dates, but adjust them to avoid overcrowding
    ax1.set_xticks(df['date'][::max(1, len(df)//10)])  # Show every 10th date or adjust accordingly
    ax1.set_xticklabels(df['date'].dt.strftime('%Y-%m-%d')[::max(1, len(df)//10)], rotation=90, ha='center', fontsize=9)

    # Adding values on top of bars (adjusting placement for better readability)
    for bar, signal in zip(bars, df["Signal"]):
        ax1.text(bar.get_x() + bar.get_width() / 2, bar.get_height() / 2, f'{signal:.2f}',
                 ha='center', va='center', fontsize=9, rotation=90)
...
def plot_signal_chart_new(df, selection, signal):
    ...
    # Adjust the width of bars (0.8 to allow for spacing between them)
    bar_width = 0.8
    
    # Plot bars
    bars = ax1.bar(df["date"], df["R_t"], color=bar_cmap(norm(df["Signal"])), width=bar_width)

    # Set tick labels alignment and rotation
    # Select only a subset of dates (e.g., every 5th date) for better readability
    date_ticks = df['date'][::5]  # Select every 5th date for ticks
    ax1.set_xticks(date_ticks)  # Use the selected dates as ticks
    ax1.set_xticklabels(date_ticks.dt.strftime('%Y-%m-%d'), rotation=90, ha='center')  # Format and rotate the labels
